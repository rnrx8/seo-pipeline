...
def _build_part_instructions(word_count_setting: str | None) -> tuple[str, str, str]:
    ...
    part1 = (
        ...
        f"今回はリード文〜H2の3つ目程度を書いてください。\n"
        f"【厳守】この時点で{p1:,}字を超えたら、そこで止めて【PART1_END】を書いてください。"
    )
    part2 = (
        f"上記の続きから、残りの約半分（H2の4つ目〜6つ目程度）を書いてください。\n"
        f"【厳守】累計が{p2:,}字を超えたら、そこで止めて【PART2_END】を書いてください。"
    )
    part3 = (
        f"上記の続きから最後のまとめセクションまで書いてください。\n"
        f"【厳守】合計が{total:,}字を超えないよう調整してください。超えそうな場合はセクションを削って構いません。\n"
        f"必ずまとめまで書き切ってください。最後に【PART3_END】と書いてください。"
    )
