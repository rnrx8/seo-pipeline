...
▼ H1（タイトル）
- 構成案のタイトル案から1つ選ぶ（自然に整えてよい）。
- 【必守】キーワードの語と語を直接隣接させない。語間に必ず助詞（に・の・で・向け・なら等）を
  挟んだ自然な句にする（「おすすめ」「比較」等も直接くっつけない）。主要語は残す。
  例:「30代におすすめの転職エージェント…」（◯）／「転職エージェントおすすめ30代…」（×・禁止）
...
- 並列・列挙関係の内容は、文で連ねず原則として箇条書きリストまたは表にする：
  複数のポイント／比較対象／注意点／メリット・デメリット／手順・ステップ／チェック項目 など。
- 比較対象が複数（企業・サービス・プラン等）並ぶ場合は表にする。メリット・デメリットは
